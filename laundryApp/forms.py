...
class UpdatePasswords(PasswordChangeForm):
    old_password = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'form-control form-control-sm rounded-0'}), label="Old Password")
    new_password1 = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'form-control form-control-sm rounded-0'}), label="New Password")
    new_password2 = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'form-control form-control-sm rounded-0'}), label="Confirm New Password")
...
class SavePrice(forms.ModelForm):
    ...
    def clean_laundry_type(self):
        ...
        try:
            if id > 0:
                price = models.Prices.objects.exclude(id=id).get(
                    laundry_type=laundry_type, delete_flag=0)
            else:
                price = models.Prices.objects.get(
                    laundry_type=laundry_type, delete_flag=0)
        except:
            return laundry_type
...
class SaveProducts(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        try:
            if id > 0:
                product = models.Products.objects.exclude(
                    id=id).get(name=name, delete_flag=0)
            else:
...
class SaveLaundry(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('code', 'client', 'contact', 'status',
                  'payment', 'total_amount', 'tendered',)
    ...
    def clean_code(self):
        ...
        if code == 'generate':
            ...
            while True:
                try:
                    check = models.Laundry.objects.get(
                        code=f"{pref}{code:05d}")
                    code = code + 1
                except:
    ...
    def save(self):
        ...
        if 'price_id[]' in self.data:
            for k, val in enumerate(self.data.getlist('price_id[]')):
                ...
                try:
                    Items.append(models.LaundryItems(
                        laundry=instance, laundry_type=prices, price=price, weight=weight, total_amount=total))
                    print("LaundryItems..")
        ...
        if 'product_id[]' in self.data:
            for k, val in enumerate(self.data.getlist('product_id[]')):
                ...
                try:
                    Products.append(models.LaundryProducts(
                        laundry=instance, product=product, price=price, quantity=qty, total_amount=total))
