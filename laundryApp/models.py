    ...
    def available(self):
        try:
            stockin = StockIn.objects.filter(
                product__id=self.id).aggregate(Sum('quantity'))
            stockin = stockin['quantity__sum']
        except:
            stockin = 0
        try:
            stockout = LaundryProducts.objects.filter(
                product__id=self.id).aggregate(Sum('quantity'))
...
class Laundry(models.Model):
    ...
    status = models.CharField(max_length=2, choices=(
        ('0', 'Pending'), ('1', 'In-progress'), ('2', 'Done'), ('3', 'Picked Up')), default=0)
    payment = models.CharField(max_length=2, choices=(
        ('0', 'Unpaid'), ('1', 'Paid')), default=0)
    date_added = models.DateTimeField(default=timezone.now)
    date_updated = models.DateTimeField(auto_now=True)
    ...
    def totalItems(self):
        try:
            Items = LaundryItems.objects.filter(
                laundry=self).aggregate(Sum('total_amount'))
    ...
    def totalProducts(self):
        try:
            Products = LaundryProducts.objects.filter(
                laundry=self).aggregate(Sum('total_amount'))
...
class LaundryItems(models.Model):
    laundry = models.ForeignKey(
        Laundry, on_delete=models.CASCADE, related_name="laundry_fk")
    laundry_type = models.ForeignKey(
        Prices, on_delete=models.CASCADE, related_name="prices_fk")
    price = models.FloatField(max_length=15, default=0)
...
class LaundryProducts(models.Model):
    laundry = models.ForeignKey(
        Laundry, on_delete=models.CASCADE, related_name="laundry_fk2")
    product = models.ForeignKey(
        Products, on_delete=models.CASCADE, related_name="product_fk")
    price = models.FloatField(max_length=15, default=0)
